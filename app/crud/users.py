    ...
    try:
        query = text("""
            SELECT u.id_usuario, u.nombre_completo, u.identificacion, 
                   ...
                   u.correo, u.tipo_contrato, u.pass_hash,
        ...
        logger.info(f"Resultado de búsqueda de usuario: {result}")
        if not result:
            logger.warning(f"No se encontró usuario con el email: {email}")
            return None
        return result
    except SQLAlchemyError as e:
        logger.error(f"Error SQL al obtener usuario por email: {str(e)}")
        raise Exception(f"Error de base de datos al obtener el usuario: {str(e)}")
    except Exception as e:
        logger.error(f"Error inesperado al obtener usuario por email: {str(e)}")
        raise Exception(f"Error inesperado al obtener el usuario: {str(e)}")
...
def get_users_by_centro(db: Session, cod_centro: int):
    try:
        query = text("""
            SELECT u.id_usuario, u.nombre_completo, u.identificacion, 
                   u.id_rol, r.nombre as nombre_rol,
                   u.correo, u.tipo_contrato, u.telefono, 
